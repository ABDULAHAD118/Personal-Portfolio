            ...
            <div className="grid-cols-auto my-10 grid gap-6">
                {serviceData.map(
                    ({ icon, title, description, link }, index) => (
                        <div
                            key={index}
                            className="hover:bg-lightHover cursor-pointer rounded-lg border border-gray-400 px-8 py-12 duration-500 hover:-translate-y-1 hover:shadow-black"
                        >
                            <Image src={icon} alt="Icon" className="w-10" />
                            <h3 className="my-4 text-lg text-gray-700">
                                {title}
                            </h3>
